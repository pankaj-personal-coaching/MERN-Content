...
const server = http.createServer((err, request, response) => {
  ...
  if (request.method === "POST" && request.url === "/api/users") {
    // POST to create a new user data.
    let body = "";

    // Collect the data from request body.
    ...
    request.on("end", () => {
      ...
      response.end(JSON.stringify({ ...responseStructure, data: userData }));
    });
  }
  // PUT
  // DELETE
